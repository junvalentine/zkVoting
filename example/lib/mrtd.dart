...
Map<dynamic, dynamic> formatDG13(final EfDG13 dg13) {
  ...
  for (var value in values) {
    ...
    if (tmp is ASN1PrintableString) {
      s[label_map[id]] = tmp.stringValue;
    } else if (tmp is ASN1UTF8String) {
      s[label_map[id]] = tmp.utf8StringValue;
    }
  }
  print(s);
  return s;
...
bool verify_active_auth(Uint8List m2, Uint8List signature, EfDG15 dg15) {
  final rawSubPubKey = dg15.aaPublicKey.rawSubjectPublicKey();
  final tvSubPubKey = TLV.fromBytes(rawSubPubKey);
  var rawSeq = tvSubPubKey.value;
  if (rawSeq[0] == 0x00) {
